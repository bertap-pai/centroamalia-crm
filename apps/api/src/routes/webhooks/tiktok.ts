...
import { fetchTikTokLeadData, mapTikTokFields } from '../../lib/tiktok-lead.js';
...
export default async function tiktokWebhookRoutes(app: FastifyInstance) {
  ...
  app.get('/api/webhooks/tiktok', async (req, reply) => {
  ...
  app.post('/api/webhooks/tiktok', async (req, reply) => {
    // Always return 200 to TikTok — never let errors propagate back
    ...
    if (!rawBody) {
      app.log.error('[tiktok-webhook] Missing raw body for HMAC verification');
      return;
    }

    // Verify HMAC-SHA256
    const expected = crypto
      .createHmac('sha256', env.TIKTOK_WEBHOOK_SECRET)
      .update(rawBody)
      .digest('hex');
    ...
    try {
      ...
      app.log.warn('[tiktok-webhook] Invalid signature header');
      return;
    }
    if (sigBuf.length !== expBuf.length || !crypto.timingSafeEqual(sigBuf, expBuf)) {
      app.log.warn('[tiktok-webhook] HMAC mismatch — discarding request');
      return;
    }

    const body = req.body as TikTokWebhookBody;
    if (body.event !== 'FORM_SUBMITTED') return;

    await processLead(app, body);
  });
}

async function processLead(
  app: FastifyInstance,
  payload: TikTokWebhookBody,
): Promise<void> {
  ...
  try {
    ...
    // Fetch lead data from TikTok Business API
    const leadData = await fetchTikTokLeadData(
      payload.advertiser_id,
      payload.lead_id,
      env.TIKTOK_ACCESS_TOKEN,
    );
    const mapped = mapTikTokFields(leadData.custom_questions ?? []);
    ...
    if (!phoneE164 && !mapped.email) {
      await app.db
        ...
        .where(eq(leadSubmissions.id, submissionId));
      app.log.warn(`[tiktok-webhook] Lead ${payload.lead_id} has no phone or email`);
      return;
    }
    ...
    const [stage] = await app.db
      ...
      .from(stages)
      .where(eq(stages.id, env.TIKTOK_DEFAULT_STAGE_ID))
      .limit(1);
    if (!stage) throw new Error(`Stage ${env.TIKTOK_DEFAULT_STAGE_ID} not found`);
    ...
    const [deal] = await app.db
      .insert(deals)
      .values({
        pipelineId: env.TIKTOK_DEFAULT_PIPELINE_ID,
        stageId: env.TIKTOK_DEFAULT_STAGE_ID,
        ownerUserId: null,
    ...
    await app.db.insert(dealStageEvents).values({
      ...
      pipelineId: env.TIKTOK_DEFAULT_PIPELINE_ID,
      fromStageId: null,
      toStageId: env.TIKTOK_DEFAULT_STAGE_ID,
      changedAt: now,
    ...
    app.log.info(`[tiktok-webhook] Lead ${payload.lead_id} processed: contact=${contactId} deal=${deal.id}`);
  } catch (err) {
    app.log.error({ err }, `[tiktok-webhook] Failed to process lead ${payload.lead_id}`);
    if (submissionId) {
      await app.db
        .update(leadSubmissions)
        .set({
